        ...
        <div className="card mt-3 ms-3" style={{ width: '18rem' }}>
            <div className="card-body">
                ...
                {emissionDataYearRange[0] === 0
                    ...
                    : <Link class="btn btn-dark text-center mb-2" to={"/CountryEmission/" + regionId + "/" + countryId} state={{ regionData, countryData }}>View Country Emissions</Link>
                }

                {temperatureDataYearRange[0] === 0
                    ? <p className="card-text">No Temperature Data Available </p>
                    : <Link class="btn btn-dark text-center mb-2" to={"/TemperatureData/" + regionId + "/" + countryId} state={{ regionData, countryData }}>View Temperature Data</Link>}

            
                {cityCount !== 0 && <Link class="btn btn-dark text-center" to={"/Cities/" + regionId + "/" + countryId} state={{ regionData, countryData }}>View Cities</Link>}
